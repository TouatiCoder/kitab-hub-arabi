...
const novelCategories = ["إثارة", "رومانسي", "خيال", "غموض", "درامي"];

export default function Novels() {
  const [catFilter, setCatFilter] = useState("الكل");
  const [search, setSearch] = useState("");
  const { data: contents, isLoading } = usePublishedContents("رواية");
  ...
  const items = (contents || []).map(c => ({
    ...
    isExclusive: c.is_premium, isComplete: c.is_complete ?? undefined,
  ...
  return (
    <div className="min-h-screen bg-background" dir="rtl">
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <h1 className="text-2xl font-extrabold text-foreground mb-2">روايات</h1>
        <p className="text-muted-foreground text-sm mb-6">روايات مكتملة ومستمرة من كتّاب عرب مميزين</p>

        <div className="flex flex-col sm:flex-row gap-3 mb-6">
          <input type="text" placeholder="ابحث عن رواية أو كاتب..." value={search} onChange={e => setSearch(e.target.value)}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 pb-20 md:pb-6">
          {isLoading ? (
            ...
            <div className="col-span-full text-center py-16 text-muted-foreground">لا توجد روايات تطابق البحث</div>
          ) : filtered.map((item, i) => (
            <Fragment key={item.id}>
              <ContentCard item={item} />
              {(i + 1) % adInterval === 0 && (i + 1) < filtered.length && Math.floor((i + 1) / adInterval) <= 3 && (
                <div className="col-span-full py-2">
                  <AdBanner slot={`novels-${Math.floor((i + 1) / adInterval)}`} format="horizontal" />
                </div>
              )}
            </Fragment>
          ))}
