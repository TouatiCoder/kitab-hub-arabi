...
export default function AdminSubscriptions() {
  ...
  const filtered = subscriptions.filter((s: any) => {
    const matchSearch = !search || s.user_name?.includes(search) || s.user_email?.includes(search);
    const matchStatus = filterStatus === "الكل" || s.status === filterStatus;
    return matchSearch && matchStatus;
  });

  const activeCount = subscriptions.filter((s: any) => s.status === "نشط").length;
  const expiredCount = subscriptions.filter((s: any) => s.status !== "نشط").length;

  const stats = [
    { label: "إجمالي المشتركين", value: subscriptions.length, icon: Crown, color: "bg-amber-500/10 text-amber-600" },
    { label: "مشتركين نشطين", value: activeCount, icon: UserCheck, color: "bg-green-500/10 text-green-600" },
    { label: "اشتراكات منتهية", value: expiredCount, icon: UserX, color: "bg-red-500/10 text-red-600" },
  ];
    ...
    <div className="min-h-screen bg-background flex" dir="rtl">
      ...
      <div className={`flex-1 flex flex-col min-h-screen transition-all ${sidebarOpen ? "mr-64" : "mr-16"}`}>
        <header className="h-16 bg-card border-b border-border flex items-center justify-between px-6 sticky top-0 z-30">
          <p className="text-sm font-bold text-foreground">إدارة الاشتراكات</p>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border bg-muted/40">
                    ...
                    <th className="text-right px-4 py-3.5 font-semibold text-muted-foreground hidden md:table-cell">الخطة</th>
                    <th className="text-right px-4 py-3.5 font-semibold text-muted-foreground hidden lg:table-cell">ينتهي في</th>
                ...
                <tbody>
                  {isLoading ? (
                    ...
                    <tr key={sub.id} className={`border-b border-border last:border-0 hover:bg-accent/40 ${idx % 2 !== 0 ? "bg-muted/10" : ""}`}>
                      <td className="px-5 py-3.5 font-medium text-foreground">{sub.user_name || "—"}</td>
                      <td className="px-4 py-3.5 text-muted-foreground hidden sm:table-cell">{sub.user_email}</td>
                      <td className="px-4 py-3.5 hidden md:table-cell">
                        <span className="text-xs font-semibold px-2.5 py-1 rounded-lg bg-primary/10 text-primary">{sub.plan}</span>
                      </td>
                      <td className="px-4 py-3.5 text-muted-foreground hidden lg:table-cell">{new Date(sub.expires_at).toLocaleDateString("ar")}</td>
                      <td className="px-4 py-3.5 font-semibold text-foreground">{sub.amount}$</td>
                      <td className="px-4 py-3.5">
                        <span className={`text-xs font-bold px-2.5 py-1 rounded-lg ${sub.status === "نشط" ? "bg-green-100 text-green-700" : "bg-amber-100 text-amber-700"}`}>{sub.status}</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
